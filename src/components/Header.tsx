...
export function Header({
  ...
  return (
    <header
      role="banner"
      className="sticky top-0 z-20 flex h-14 w-full items-center justify-between border-b border-slate-200/90 bg-white/95 px-2.5 sm:px-6 backdrop-blur-md transition-colors dark:border-slate-800 dark:bg-slate-900/95"
    >
      {/* Left Area: Sidebar Toggle & View Switcher */}
      <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
        ...
        {/* Simple & Clear View Switcher Segmented Control */}
        <nav
          role="tablist"
          aria-label="Workspace views"
          className="flex items-center rounded-xl bg-slate-100/90 p-0.5 sm:p-1 border border-slate-300/80 dark:border-slate-700/80 dark:bg-slate-800/90"
        >
          <button
            role="tab"
            ...
            className={`flex items-center justify-center gap-1 sm:gap-1.5 rounded-lg px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs font-semibold transition-all focus-visible:ring-2 focus-visible:ring-emerald-600 ${
      ...
      {/* Right Area: Simple Action Buttons */}
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        ...
        {onSyncNow && (
          <button
            ...
            title={`Sync Status: ${lastSyncedText}. Tap to refresh live data.`}
            className="flex h-8 w-8 sm:h-9 sm:w-auto sm:px-2.5 items-center justify-center gap-1.5 rounded-xl border border-slate-300/80 bg-slate-50/80 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-900 disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-emerald-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100"
          >
            <RefreshCw
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl border border-slate-300/80 text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-emerald-600 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-100"
          >
            {resolvedTheme === "dark" ? (
