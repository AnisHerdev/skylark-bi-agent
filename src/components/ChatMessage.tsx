...
export function ChatMessage({ message }: { message: ChatMessageType }) {
  ...
  return (
    <div className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-6`}>
      <div
        className={`flex max-w-[92%] sm:max-w-[85%] items-start gap-3 ${
      ...
      >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <div
            className={`rounded-2xl px-5 py-4 text-sm leading-relaxed shadow-xs transition-colors ${
              isUser
            ...
            {isUser ? (
              ...
              <div className="prose prose-slate dark:prose-invert max-w-none text-sm leading-relaxed">
                <ReactMarkdown
                  ...
                  components={{
                    table: ({ ...props }) => (
                      <div className="bi-markdown-table-wrapper my-3">
                        <table className="bi-markdown-table" {...props} />
                      </div>
                    ),
                    th: ({ ...props }) => (
                      <th className="px-3 py-2 text-left font-semibold text-slate-800 dark:text-slate-200 bg-slate-100/60 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700" {...props} />
                    ),
                    td: ({ ...props }) => (
                      <td className="px-3 py-1.5 border-b border-slate-100 dark:border-slate-800/60 text-slate-600 dark:text-slate-300" {...props} />
                    ),
                    h1: ({ ...props }) => (
                      <h1 className="text-base font-bold text-slate-900 dark:text-slate-50 mt-4 mb-2 first:mt-0" {...props} />
                    ),
                    h2: ({ ...props }) => (
                      <h2 className="text-sm font-bold text-slate-800 dark:text-slate-100 mt-3 mb-1.5" {...props} />
                    ),
                    h3: ({ ...props }) => (
                    ...
                    code: ({ ...props }) => (
                      <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs text-emerald-700 dark:bg-slate-800 dark:text-emerald-300" {...props} />
                    ),
                  }}
                >
                  {message.content}
